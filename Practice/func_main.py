...
def open_file():  # функция открытия БД при запуске приложения
    if (not os.path.isfile("base.json")):  # проверка на сущетвование БД
        ...
        messagebox.showerror(
            "Error!", "В базе данных нет контактов.\nВнесите хотя бы один контакт.")
        fio = input()
        print("Введите номер телефона " + fio)
        # c реализацией бд есть нюансы, поэтому тут не идеально все.
        number_phone = input()
        count_contacks = '0'
        contacks = {"contacks":  {"count_contacks": count_contacks, count_contacks: {
            "fio": fio, "number_phone": str(number_phone)}}}
...
def write_file():  # функция записи контактов в БД
    ...
    label_output = gui.Label(
        window1, text="Заполните поля, а затем нажмите кнопку \"Создать\"", font=("Comic Sans MS", 12))
    label_output.pack(padx=0, pady=1)

    fio_text = gui.StringVar()
    fio_text.set("FIO")
    input_fio = gui.Entry(
        window1, width=50, bg="dark sea green", textvariable=fio_text)
    ...
    button0 = gui.Button(window1, text="Отмена", font=(
        "Comic Sans MS", 12), bg="orange red", command=window1.destroy)
    button0.place(width=80, height=30, relx=0.99, rely=0.99, anchor="se")

    sleep_var = gui.IntVar()
    button = gui.Button(window1, text="Создать", font=(
        "Comic Sans MS", 12), bg="green", command=lambda: sleep_var.set(1))
